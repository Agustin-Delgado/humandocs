...
// generated JSON, committed, rendered by <ApiReference>).
//
...
// Mirrors the ApiProp/ApiPart/ComponentApi contract in src/lib/api/types.ts
// (kept local: the CLI compiles standalone from src/cli).
interface ApiProp {
	name: string;
	type: string;
	required: boolean;
...
export async function runExtractApi(options: ExtractApiOptions): Promise<void> {
	...
	for (const [flag, dir] of [
		...
		if (!(await exists(dir))) {
			console.error(`humandocs extract-api: ${flag} directory does not exist: ${dir}`);
			process.exitCode = 1;
			return;
		}
	}
